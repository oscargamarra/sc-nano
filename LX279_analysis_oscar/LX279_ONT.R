#.libPaths(c("/hpc/pmc_holstege/rstudio/oscar_clonetracer/R/x86_64-pc-linux-gnu-library/4.4",
...
CFG$data_dir <- "~/LX279_gene_expression/ONT/analysis_nanopore_LX279/genes"  # where to read data from
CFG$output_dir <- "~/LX279_gene_expression/ONT/analysis_nanopore_LX279/genes"  # where to write data to
...
countsdir <- paste0(CFG$data_dir, "/LX279_sup_complete.gene_expression.counts.tsv")  # what is actually on disk
...
sop <- read.delim("~/sop_input/nuc/clusters.tsv", header=TRUE, row.names = 1)
rownames(sop) <- paste0(lib,"_",rownames(sop))
rownames(sop) <- gsub("-1", "", rownames(sop), fixed = TRUE)
...
DimPlot(srat, reduction = "pca", group.by = 'patient', pt.size = 1)
...
p_phase | DimPlot(srat, group.by = 'patient')
...
srat <- FindClusters(srat, resolution = 1.4, algorithm = 1)
## Modularity Optimizer version 1.3.0 by Ludo Waltman and Nees Jan van Eck
## 
## Number of nodes: 1296
## Number of edges: 37806
...
## scDblFinder ----
sce.lr <- scDblFinder(GetAssayData(srat, assay = "RNA", slot = "counts"), 
                   iter = 10, 
                   includePCs = 14,
                   dims = 20)
...
#Simplify cell types
cell_percentage_ont <- sum(table(srat$singler))
...
srat@meta.data <- srat@meta.data %>%
  mutate(
    cells_by_patient = case_when(
      patient_ilmn == '1' & singler == 'B_cell' ~ 'P0 B cell',
      patient_ilmn == '1' & singler == 'Monocyte' ~ 'P0 Monocyte',
      patient_ilmn == '1' & singler == 'NK_cell' ~ 'P0 NK cell',
      patient_ilmn == '1' & singler == 'Other' ~ 'P0 Other',
      patient_ilmn == '1' & singler == 'Pro-B_cell_CD34+' ~ 'P0 Pro-B cell CD34+',
      patient_ilmn == '1' & singler == 'T_cells' ~ 'P0 T cells',
      patient_ilmn == '0' & singler == 'B_cell' ~ 'P1 B Cell',
      patient_ilmn == '0' & singler == 'Monocyte' ~ 'P1 Monocyte',
      patient_ilmn == '0' & singler == 'NK_cell' ~ 'P1 NK cell',
      patient_ilmn == '0' & singler == 'Other' ~ 'P1 Other',
      patient_ilmn == '0' & singler == 'Pro-B_cell_CD34+' ~ 'P1 Pro-B cell CD34+',
      patient_ilmn == '0' & singler == 'T_cells' ~ 'P1 T cells',
      TRUE ~ 'NA'
    )
  )
...
df <- data.frame(cell = colnames(srat), type = srat$cells_by_patient)
...
## it there:
outdir <- paste0(CFG$output_dir, "/infercnv_celltypes")
dir.create(outdir)
...
infercnv_obj <- infercnv::run(infercnv_obj, out_dir = outdir,
                              ...
                              num_threads = 2, output_format = "png", window_length = 101,
