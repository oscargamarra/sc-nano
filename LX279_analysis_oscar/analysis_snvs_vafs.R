...
cutoff$NAs <- 20 #Maximum of n% of NAs, in other words, highly covered
...
rownames(nuc.matrix[nuc.unfiltered$NAs<cutoff$NAs & nuc.unfiltered$sd > cutoff$sds,])
...
DimPlot(srat.lr, group.by = 'patient_ont', cols = c('#a6e0e9', '#f64941')) + labs(title = 'Annotation using souporcell') |
...
p0.healthy <- colnames(pat0.vafs.filtered)[column_order(p0.ht)[[2]]]
p0.tumor <- colnames(pat0.vafs.filtered)[column_order(p0.ht)[[1]]]
...
test[WhichCells(srat.lr, cells = p0.healthy)] <- "p0.healthy"
test[WhichCells(srat.lr, cells = p0.tumor)] <- "p0.tumor"
...
#In the reference the patients are inversed
test[WhichCells(srat.lr, cells = p1.healthy)] <- "p1.tumor"
test[WhichCells(srat.lr, cells = p1.tumor)] <- "p1.healthy"
...
srat.lr$test_binary <- ifelse(srat.lr$test %in% c("p0.healthy", "p1.healthy"), 0, 
                          ifelse(srat.lr$test %in% c("p0.tumor", "p1.tumor"), 1, NA))


valid_indices <- !is.na(srat.lr$test_binary)  # Get indices where prediction is not NA
predicted <- srat.lr$test_binary[valid_indices]  # Keep only valid predictions
...
k2_nuc <- read.table(file = paste0(sop,'clusters.tsv'), header = TRUE)
...
##k2 ----
k2_tsv.p0 <- read.table(file = paste0(sop,'p0/clusters_k2.tsv'), header = TRUE)
k2_tsv.p1 <- read.table(file = paste0(sop,'p1/clusters_k2.tsv'), header = TRUE)
